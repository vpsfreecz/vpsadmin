...
switch ($_GET["action"]) {
		...
		case 'mass_management_exec':
			...
			$xtpl->form_create('?page=adminvps&action=mass_management_exec2&cmd=' . $_POST["cmd"], 'post');
			$xtpl->table_td(
			...
			foreach ($_POST["vpses"] as $veid)
				$vpses[] = '<a href="?page=adminvps&action=info&veid='.$veid.'">'.$veid.'</a>';
...
if ($mass_management && $_SESSION["is_admin"]) {
	$xtpl->title(_("Mass managenent"));
	
	$xtpl->table_title(_("Filters"));
	$xtpl->form_create('', 'get');
	
	$xtpl->table_td('<input type="hidden" name="page" value="adminvps">
	...
	$xtpl->form_create('?page=adminvps&action=mass_management_exec', 'post');
	...
	while ($row = $db->fetch_array($res)) {
		...
		$xtpl->table_td('<a href="?page=adminvps&action=info&veid='.$vps->veid.'">'.$vps->veid.'</a>');
		$xtpl->table_td('<a href="?page=adminvps&action=mass_management&n[]='.$vps->ve['server_name'].'">'.$vps->ve["server_name"].'</a>');
		...
		$xtpl->table_td('<a href="?page=adminvps&action=mass_management&t[]='.$row["templ_id"].'">'.$row["templ_label"].'</a>');
		$xtpl->table_td($vps->ve["vps_nproc"], false, true);
		$xtpl->table_td(sprintf('%4d MB', $vps->ve["vps_vm_used_mb"]), false, true);
