...
test('email verification protects a new browser and preserves known-device login', async ({ browser, request }) => {
  ...
  await expect(logoutButton(page)).toHaveValue(`Logout (${account.username})`);
  ...
  await form.locator('input[type="submit"]').click();
  await expect(page.locator('#content-in')).toContainText('Email verification settings updated');
  ...
  try {
    ...
    await unknown.getByRole('button', { name: 'Verify and sign in' }).click();
    await expect(logoutButton(unknown)).toHaveValue(`Logout (${account.username})`);
  } finally {
    await fresh.close();
    await known.close();
  }
