...
''
  ...
  def wait_for_running_nodectld(node)
    ...
    wait_until_block_succeeds(name: "nodectld supervised on #{node.name}") do
      ...
      _, status = node.succeeds('nodectl status', timeout: 180)
  ...
  def set_export_enabled(services, admin_user_id:, export_id:, enabled:, timeout: 300)
    response = services.api_ruby_json(code: <<~RUBY)
      #{api_session_prelude(admin_user_id)}

      export = Export.find(#{Integer(export_id)})
      chain, updated = VpsAdmin::API::Operations::Export::Update.run(
        export,
        enabled: #{enabled ? 'true' : 'false'}
      )
  ...
  def add_export_host(services, admin_user_id:, export_id:, ip_address_id:, timeout: 300)
    response = services.api_ruby_json(code: <<~RUBY)
      #{api_session_prelude(admin_user_id)}
