...
The storage helpers in `tests/suite/storage/remote-common.nix` support
...
STORAGE_TOPOLOGY_FIXTURE_DIR=/tmp/storage-fixtures \
  ./test-runner.sh test storage/repeated-rollback-branching
```
...
bundle exec ruby bin/storage-topology-fixture \
  ...
  --output ../tests/suite/storage/fixtures/example.json \
...
bundle exec ruby bin/storage-topology-check \
  ../tests/suite/storage/fixtures/synthetic-good-topology.json
```
...
`tests/suite/storage/topology-fixture-replay.nix`, which validates:
