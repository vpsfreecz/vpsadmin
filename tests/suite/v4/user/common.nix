...
+ ''
  ...
  def user_create_namespace_hook_source
    <<~'RUBY'
  ...
  def ensure_user_create_namespace_hook(services)
    services.api_ruby_json(code: <<~RUBY)
      #{user_create_namespace_hook_source}
      puts JSON.dump(status: true)
    RUBY
  end
  ...
  def user_sessions_for(services, user_id, label: 'user-token')
    services.api_ruby_json(code: <<~RUBY)
      puts JSON.dump(UserSession.where(
        user_id: #{Integer(user_id)},
        label: #{label.inspect}
      ).order(:id).map do |session|
        {
          id: session.id,
          label: session.label,
  ...
  def create_user_vps_export_dns_fixture(services, login:, password:)
    ...
    export_dataset = create_top_level_dataset(
      services,
      admin_user_id: created.fetch('user_id'),
      pool_id: PoolId.for(services, 'tank/user-primary'),
      dataset_name: "#{login}-export",
      refquota: nil
    )
    ensure_private_export_network_with_ips(
      services,
      admin_user_id: admin_user_id,
      dataset_id: export_dataset.fetch('dataset_id')
    )
