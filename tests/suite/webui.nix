  ...
  let
      ...
      def run_playwright(script_name, *specs, timeout: 1800)
        ...
        begin
          services.succeeds(<<~SH, timeout: timeout)
            ...
            export VPSADMIN_WEBUI_VERSION="$(${pkgs.jq}/bin/jq --raw-output .version /etc/vpsadmin/build-info.json)"
    ...
    fixtureScript = pkgs.writeText "vpsadmin-webui-browser-fixtures.rb" ''
      ...
      networking_ip_specs = {
        ...
        release_other_owner: ['203.0.113.178', secondary_user, nil, nil],
          ...
          describe 'webui networking and DNS browser flow' do
            it 'passes Playwright networking and DNS tests' do
              preferences = services.api_ruby_json(code: <<~RUBY)
                ...
                env = Environment.find(${toString seed.environment.id})
                [user, User.find_by!(login: 'webui-user-secondary')].each do |owner|
                  config = owner.environment_user_configs.find_by!(environment: env)
                  ...
                  config.reallocate_resource!(
                    :ipv4, usage, user: owner, save: true,
