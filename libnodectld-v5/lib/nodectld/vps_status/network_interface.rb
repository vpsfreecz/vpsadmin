module NodeCtld
  class VpsStatus::NetworkInterface
    ...
    def initialize(opts)
      ...
      @last_bytes_in = opts.fetch('bytes_in_readout')
      @last_bytes_out = opts.fetch('bytes_out_readout')
      @last_packets_in = opts.fetch('packets_in_readout')
      @last_packets_out = opts.fetch('packets_out_readout')
      @bytes_in = 0
      @bytes_out = 0
      @packets_in = 0
      @packets_out = 0
