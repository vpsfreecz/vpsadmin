defmodule VpsAdmin.Cluster.TransactionTest do
  ...
  alias VpsAdmin.Cluster.{Command, Transaction, Query}
  ...
  defmodule ResourceLocking do
    ...
    def create(ctx, _opts) do
      location = Cluster.Repo.insert!(%Schema.Location{label: "Test", domain: "test"})
  ...
  defmodule RunPipeline do
    ...
    def create(ctx, pid) do
      ...
      |> run(fn ctx ->
        ...
        |> Context.put(
          :location,
          Cluster.Repo.insert!(%Schema.Location{label: "Test", domain: "test"})
        )
      end)
      |> lock(fn ctx -> ctx.data.location end)
  ...
  test "resource locking" do
    ...
    assert length(Query.Transaction.Chain.locks(chain)) == 1
  ...
  test "refuses to lock resource locked by another transaction" do
    location = Cluster.Repo.insert!(%Cluster.Schema.Location{label: "Test", domain: "test"})
