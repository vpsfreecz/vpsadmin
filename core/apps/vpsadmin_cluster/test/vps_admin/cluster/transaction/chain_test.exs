defmodule VpsAdmin.Cluster.Transaction.ChainTest do
  ...
  test "can be closed on success" do
    ...
    assert Query.Transaction.Chain.locks(chain) == []
    assert Query.get(Schema.Location, data.loc1)
    assert Query.get(Schema.Location, data.loc2)
    assert Query.get(Schema.Location, data.loc3)
  end
  ...
  test "can be closed on error" do
    ...
    assert Query.Transaction.Chain.locks(chain) == []
    refute Query.get(Schema.Location, data.loc1)
    refute Query.get(Schema.Location, data.loc2)
    refute Query.get(Schema.Location, data.loc3)
  end
end
