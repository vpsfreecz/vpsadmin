defmodule VpsAdmin.Cluster.Transaction.Confirmation do
  ...
  include several columns, see `VpsAdmin.Cluster.Migration.confirmation_columns/0`
  and `VpsAdmin.Cluster.Schema.Transaction.Confirmation/confirmation_fields/0`.

  Changed rows are marked so that you can query for confirmed or latest data,
  based on your context, see `VpsAdmin.Cluster.Query`. Changes are not permanent
  ...
  alias VpsAdmin.Cluster.{Query, Schema}
  require Ecto.Query
  ...
  def change(ctx, %Ecto.Changeset{} = changeset, changes) do
    ...
    row = Cluster.Repo.update!(changeset)
  ...
  def confirm(cnf, :ok) do
    ...
    case cnf.type do
      :insert ->
        ...
        |> Cluster.Repo.update_all(set: [row_state: states[:confirmed]])
      ...
      :update ->
        ...
        Ecto.Query.from(cnf.table, where: ^pks)
        |> Cluster.Repo.update_all(set: changes ++ [
    ...
    {:ok, _} = cnf
      ...
      |> Query.Transaction.Confirmation.update()
  end
