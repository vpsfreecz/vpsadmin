...
$adminActions = ['new', 'show', 'edit', 'create', 'update', 'close', 'notify', 'release', 'exempt', 'campaign_notices', 'notices', 'selection'];
    ...
    if (in_array($action, $mutations, true) && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        switch ($action) {
            ...
            case 'notify':
                ...
                break;
            case 'release':
                $attempt = $campaign->release();
                $message = ip_release_attempt_label($attempt->state);
                break;
            case 'keep':
