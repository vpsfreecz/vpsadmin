...
function dnsServerZoneServerInfo($serverZone)
{
    return '<div class="dns-zone-server-name">' . h($serverZone->dns_server->name) . '</div>'
        . dnsServerZoneDefinitionList([
            [_('IPv4 address'), $serverZone->dns_server->ipv4_addr ? h($serverZone->dns_server->ipv4_addr) : '-'],
            [_('IPv6 address'), $serverZone->dns_server->ipv6_addr ? h($serverZone->dns_server->ipv6_addr) : '-'],
        ]);
}
...
function dnsServerZoneDefinitionList($items)
{
    $ret = '<dl class="inline dns-zone-server-details">';
