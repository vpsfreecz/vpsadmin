...
function ip_release_list()
{
    ...
    if (isAdmin()) {
        foreach ([
            [_('Total'), _('All allocations recorded in this campaign, including released or changed addresses. Each subnet counts as one allocation.')],
            [_('Release'), _('Addresses currently eligible for release in an open campaign, plus releases in progress.')],
            [_('Keep'), _('Addresses in use, exempted by an admin, or kept with a reason honored under the current policy. Released, changed and unresolved addresses in closed campaigns count only toward the total.')],
        ] as [$label, $description]) {
            $xtpl->table_add_category('<span title="' . h($description) . '">' . h($label) . '</span>');
        }
    }
    foreach ($rows as $row) {
        ...
        if (isAdmin()) {
            ...
            $xtpl->table_td((int) $row->total_ip_count, false, true);
            $xtpl->table_td((int) $row->to_release_ip_count, false, true);
            $xtpl->table_td((int) $row->kept_ip_count, false, true);
        }
        $xtpl->table_tr();
    }
    if (!count($rows)) {
        $xtpl->table_td(isAdmin() ? _('No IP release campaigns.') : _('No IP release requests.'), false, false, isAdmin() ? 7 : 2);
        $xtpl->table_tr();
    }
...
{
    $label = h(_('Select all addresses on this page'));
    return '<input type="checkbox" data-select-all title="' . $label . '" aria-label="' . $label . '">';
}
...
function ip_release_all_rows($action, $params = [])
{
    $rows = [];
    $from = 0;
    $limit = $params['limit'] ?? IP_RELEASE_PAGE_SIZE;
    do {
        $page = $action->call(array_merge($params, ['limit' => $limit, 'from_id' => $from]));
        foreach ($page as $row) {
            $rows[] = $row;
            $from = $row->id;
        }
...
function ip_release_new()
{
    ...
    $xtpl->table_add_category(ip_release_selection_header());
    foreach ([_('IP address'), _('User'), _('Network'), _('Location')] as $label) {
        $xtpl->table_add_category($label);
    }
    $selectionAction = h(ip_release_url('selection') . $selectionUrl);
    $xtpl->table_td('<button type="submit" name="selection_action" value="all" formaction="' . $selectionAction . '">' . _('Select all matches') . '</button> '
...
function ip_release_counts($campaign)
{
    $counts = [
        [_('Total'), $campaign->total_ip_count, _('All allocations recorded in this campaign, including released or changed addresses. Each subnet counts as one allocation.')],
        [_('To be released'), $campaign->to_release_ip_count, _('Addresses currently eligible for release in an open campaign, plus releases in progress.')],
        [_('Kept'), $campaign->kept_ip_count, _('Addresses in use, exempted by an admin, or kept with a reason honored under the current policy. Released, changed and unresolved addresses in closed campaigns count only toward the total.')],
    ];
    return implode('<br>', array_map(fn($count) => '<span title="' . h($count[2]) . '">'
        . h($count[0]) . ': ' . (int) $count[1] . '</span>', $counts));
}
...
function ip_release_summary($record)
{
    ...
    $xtpl->table_title(_('Campaign settings'));
    ip_release_info(_('IPs'), ip_release_counts($record));
...
function ip_release_action_form($campaign, $action)
{
    ...
    if ($action === 'release') {
        ...
        ip_release_info(_('Cleanup'), h(_('All selected addresses remain owned until the whole release succeeds. If it rolls back, none are released. Addresses in a running release cannot be retained.')));
    } elseif ($action === 'close') {
...
function ip_release_attempt_label($state)
{
    return [
        'running' => _('Release in progress'),
        'rolling_back' => _('Rolling back; ownership is retained'),
        'released' => _('All selected addresses released'),
        'failed' => _('Release rolled back; no addresses released'),
        'preparation_failed' => _('Release could not start; no addresses released'),
        'empty' => _('No eligible addresses'),
        'attention' => _('Transaction chain needs administrator attention'),
    ][$state] ?? $state;
}

function ip_release_chain_link($id)
{
    return $id ? '<a href="?page=transactions&chain=' . (int) $id . '">'
        . h(sprintf(_('Transaction chain #%d'), $id)) . '</a>' : '';
}

function ip_release_attempt_error($attempt)
{
    if ($attempt->error === 'resource_locked') {
        return h(sprintf(_('Resource %s #%d is busy. Try again after its transaction finishes.'), $attempt->blocked_resource, $attempt->blocked_resource_id))
            . ($attempt->blocked_chain_id ? '<br>' . ip_release_chain_link($attempt->blocked_chain_id) : '');
    }
    if ($attempt->error === 'database_busy') {
        return h(_('The database was busy. Try releasing the addresses again.'));
    }
    return h($attempt->error ?? '');
}

function ip_release_attempts($campaign)
{
    global $api, $xtpl;

    if (!$campaign->latest_release_attempt_id) {
        return;
    }
    $latest = $campaign->latest_release_attempt;
    $xtpl->table_title(_('Latest release attempt'), 'ip-release.latest-attempt');
    ip_release_info(_('Result'), '<strong>' . h(ip_release_attempt_label($latest->state)) . '</strong>');
    ip_release_info(_('IPs'), (int) $latest->ip_count);
    if ($latest->transaction_chain_id) {
        ip_release_info(_('Transaction chain'), ip_release_chain_link($latest->transaction_chain_id));
    }
    if ($latest->error) {
        ip_release_info(_('Error'), ip_release_attempt_error($latest));
    }
    if ($latest->state === 'attention') {
        ip_release_info(_('Recovery'), h(_('Inspect and recover the transaction chain before starting another release.')));
    }
    $xtpl->table_out();

    $xtpl->table_title(_('Release attempts'), 'ip-release.attempts');
    foreach ([_('Started at'), _('Administrator'), _('IPs'), _('Result'), _('Transaction chain')] as $label) {
        $xtpl->table_add_category($label);
    }
    foreach (ip_release_all_rows($api->ip_release_campaign($campaign->id)->attempt->list, ['limit' => 100]) as $attempt) {
        $xtpl->table_td(h(tolocaltz($attempt->created_at, 'Y-m-d H:i T')));
        $xtpl->table_td(ip_release_user($attempt->created_by_id, $attempt->created_by_login));
        $xtpl->table_td((int) $attempt->ip_count, false, true);
        $xtpl->table_td(h(ip_release_attempt_label($attempt->state)) . ($attempt->error ? '<br>' . ip_release_attempt_error($attempt) : ''));
        $xtpl->table_td(ip_release_chain_link($attempt->transaction_chain_id));
        $xtpl->table_tr();
    }
    $xtpl->table_out();
}
