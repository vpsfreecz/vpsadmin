...
def tr(t, val)
  ...
  raise NotFound, "[#{t}.#{val} not found]" unless n
...
rs.each_hash do |row|
  if options[:dry_run]
    ...
    puts "\t#{row['vps_privvmpages']} -> #{$translation[:ram][row['vps_privvmpages'].to_i]} (#{tr(:ram,
                                                                                                  row['vps_privvmpages'])})"
    puts "\t#{row['vps_diskspace']} -> #{$translation[:hdd][row['vps_diskspace'].to_i]} (#{tr(:hdd,
                                                                                              row['vps_diskspace'])})"
    puts "\t#{row['vps_cpulimit']} -> #{$translation[:cpu][row['vps_cpulimit'].to_i]} (#{tr(:cpu,
                                                                                            row['vps_cpulimit'])})"
  else
