    ...
    def set_user_data(vps, opts)
      ...
      if opts[:user_data]
        if opts[:user_data].user_id != vps.user_id
          raise Exception::OperationError, 'Access denied to VPS user data'
        elsif !vps.os_template.support_user_data?(opts[:user_data])
          raise Exceptions::OperationError, "OS template #{vps.os_template.label} does not support given ser data"
        end
      end
