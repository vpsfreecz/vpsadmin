...
RSpec.describe IpReleaseCampaign do
  ...
  def setup_campaign
    connection_thread do
      committed[:node] = SpecSeed.node.reload.attributes
      committed[:node_status] = NodeCurrentStatus.find_by(node: SpecSeed.node)&.attributes
      committed[:node_versions] = PaperTrail::Version.where(item_type: 'Node', item_id: SpecSeed.node.id).pluck(:id)
  ...
  after do
    connection_thread do
      attempts = IpReleaseAttempt.where(ip_release_campaign_id: committed[:campaign_id])
      chains = TransactionChain.where(id: attempts.select(:transaction_chain_id))
      transactions = Transaction.where(transaction_chain_id: chains.select(:id))
      TransactionConfirmation.where(transaction_id: transactions.select(:id)).delete_all
      ResourceLock.where(locked_by_type: 'TransactionChain', locked_by_id: chains.select(:id)).delete_all
      transactions.delete_all
      chains.delete_all
      IpReleaseAttemptAddress.where(ip_release_attempt_id: attempts.select(:id)).delete_all
      attempts.delete_all
      ...
      if committed[:node]
        Node.where(id: committed[:node]['id']).update_all(committed[:node])
        statuses = NodeCurrentStatus.where(node_id: committed[:node]['id'])
        if committed[:node_status]
          statuses.update_all(committed[:node_status])
        else
          statuses.delete_all
        end
        PaperTrail::Version.where(item_type: 'Node', item_id: committed[:node]['id'])
                           .where.not(id: committed[:node_versions]).delete_all
        SpecSeed.node.reload
      end
  ...
  it 'returns a committed active attempt even when the caller has an older database snapshot' do
    setup_campaign
    snapshot = Queue.new
    resume = Queue.new
    worker = connection_thread do
      described_class.transaction do
        ...
        expect(campaign.ip_release_attempts.count).to eq(0)
        snapshot << true
        resume.pop
        campaign.release!(actor: SpecSeed.admin).id
      end
    end
    Timeout.timeout(10) { snapshot.pop }
    attempt_id = connection_thread do
      described_class.find(committed[:campaign_id]).release!(actor: SpecSeed.admin).id
    end.value
    resume << true
    expect(Timeout.timeout(20) { worker.value }).to eq(attempt_id)
    connection_thread do
      expect(IpReleaseAttempt.where(ip_release_campaign_id: committed[:campaign_id]).count).to eq(1)
    end.value
  ensure
  ...
  it 'does not release while a user lifecycle change owns the user resource lock' do
    ...
    connection_thread do
      described_class.find(committed[:campaign_id]).release!(actor: SpecSeed.admin)
      expect(IpAddress.find(committed[:ip_id]).user_id).to eq(SpecSeed.user.id)
      expect(described_class.find(committed[:campaign_id]).latest_release_attempt.state).to eq('preparation_failed')
    end.value
    resume << true
  ...
  it 'waits for a concurrent DNS transfer writer and cleans its newly committed grant' do
    ...
    workers << connection_thread do
      ...
      described_class.transaction do
        ...
        expect(result.last_result).to eq('releasing'), result.ip_release_campaign.latest_release_attempt.error
        ip = IpAddress.find(committed[:ip_id])
  ...
  it 'blocks a stale quota writer while release confirmations are pending' do
    ...
    worker = connection_thread do
      EnvironmentUserConfig.transaction do
        ...
        snapshot << true
        resume.pop
        expect { config.adjust_resource!(:ipv4, delta: 1, user: SpecSeed.user, save: true) }.to raise_error(ResourceLocked)
      end
    end
    Timeout.timeout(10) { snapshot.pop }
    connection_thread do
  ...
  %i[close release].each do |action|
    it "rejects bulk exemptions after a concurrent #{action} commits" do
      setup_campaign
      locked = Queue.new
      resume = Queue.new
      workers = []
      workers << connection_thread do
        ...
        campaign.with_lock do
          locked << true
          resume.pop
          action == :close ? campaign.close!(actor: SpecSeed.admin) : campaign.release!(actor: SpecSeed.admin)
        end
      end
      Timeout.timeout(10) { locked.pop }
      workers << connection_thread do
        campaign = described_class.find(committed[:campaign_id])
        expect do
          campaign.exempt!(ids: [committed[:item_id]], reason: 'Too late', actor: SpecSeed.admin)
        end.to raise_error(described_class::Error, action == :close ? 'closed' : 'already_released')
      end
      expect(workers.last.join(0.1)).to be_nil
      resume << true
      Timeout.timeout(20) { workers.each(&:value) }
      connection_thread do
        expect(IpReleaseRequestAddress.find(committed[:item_id]).exempted_at).to be_nil
      end.value
    ensure
      resume << true if resume
      workers&.each { |thread| thread.join(25) || thread.kill.join }
  ...
  %i[keep exemption bulk_exemption policy close release].each do |action|
    it "serializes release after a concurrent #{action} and reloads the current state" do
      ...
      workers << connection_thread do
        ...
        campaign.with_lock do
          ...
          when :bulk_exemption
            campaign.exempt!(ids: [committed[:item_id]], reason: 'Reservation', actor: SpecSeed.admin)
          when :policy
            campaign.ip_release_requests.first.keep!(ids: [committed[:item_id]], reason: 'Migration', actor: SpecSeed.user)
      ...
      connection_thread do
        ip = IpAddress.find(committed[:ip_id])
        config = EnvironmentUserConfig.find(committed[:config_id])
        releasing = %i[policy release].include?(action)
        expect(ip.user_id).to eq(SpecSeed.user.id)
        expect(config.ipv4).to eq(before_usage)
        item = IpReleaseRequestAddress.find(committed[:item_id])
        expect(item.released_at).to be_nil
        expect(item.protection == 'releasing').to eq(releasing)
        if releasing
          expect(IpReleaseAttempt.where(ip_release_campaign_id: committed[:campaign_id]).count).to eq(1)
        end
      end.value
