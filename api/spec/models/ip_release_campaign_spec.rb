...
RSpec.describe IpReleaseCampaign do
  ...
  it 'releases a full campaign populated by managed network additions exactly once' do
    ...
    expect(c.ip_release_request_addresses.where.not(released_at: nil).count).to eq(101)
    expect(IpAddress.where(id: ips.map(&:id)).where.not(user_id: nil)).to be_empty
    expect(config.reload.ipv4).to eq(usage - 101)
  end
  ...
  it 'filters public and private families, owners and overlapping locations without duplicates' do
    ...
    2.times { c.release!(actor: SpecSeed.admin) }
    expect(private_ip.reload.user_id).to be_nil
    expect(config.reload.ipv4_private).to eq(before_private - private_ip.size)
