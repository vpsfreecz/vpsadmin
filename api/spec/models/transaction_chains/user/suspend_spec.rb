...
RSpec.describe TransactionChains::User::Suspend do
  ...
  it 'sends suspend mail, stops VPSes, and disables user DNS state for target transitions' do
    ...
    expect_deferred_event!(chain, 'user.suspended')
    ...
    event = expect_completed_event!('user.suspended', user: fixture.fetch(:user))
    expect(event.source_class).to eq('ObjectState')
    expect(event.parameters).to include('operation_id' => chain.id, 'state' => 'suspended')
