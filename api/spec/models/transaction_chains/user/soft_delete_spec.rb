...
RSpec.describe TransactionChains::User::SoftDelete do
  ...
  it 'sends mail, cascades runtime disables, records VPS soft-delete, and closes auth state' do
    ...
    expect_deferred_event!(chain, 'user.soft_deleted')
    ...
    event = expect_completed_event!('user.soft_deleted', user:)
    expect(event.source_class).to eq('ObjectState')
    expect(event.parameters).to include('operation_id' => chain.id, 'state' => 'soft_delete')
