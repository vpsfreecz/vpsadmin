...
RSpec.describe TransactionChains::Vps::Clone::OsToOs do
  ...
  it 'rejects cloned descendant diskspace that exceeds the user quota' do
    fixture = create_clone_fixture
    vps = fixture.fetch(:vps)
    resource = UserClusterResource.find_by!(
      user: vps.user,
      environment: fixture.fetch(:dst_node).location.environment,
      cluster_resource: ClusterResource.find_by!(name: 'diskspace')
    )
    used = ClusterResourceUse.where(user_cluster_resource: resource, enabled: true)
                             .where.not(confirmed: ClusterResourceUse.confirmed(:confirm_destroy))
                             .sum(:value)

    resource.update!(value: used + fixture.fetch(:root_dip).diskspace)
    ...
    expect do
      described_class.fire(
        ...
        fixture.fetch(:dst_node),
        user: vps.user,
        hostname: 'over-quota-descendant-clone',
        stop: false,
        dataset_plans: false,
        resources: true,
        features: false
      )
    end.to raise_error(VpsAdmin::API::Exceptions::ClusterResourceAllocationError)

    expect(Vps.where(hostname: 'over-quota-descendant-clone')).to be_empty
  end
