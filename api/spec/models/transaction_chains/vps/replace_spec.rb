...
RSpec.describe TransactionChains::Vps::Replace do
  around do |example|
  ...
  it 'returns Os for vpsadminos to vpsadminos replacements' do
    vps, dst_node = create_vps_fixture

    expect(described_class.chain_for(vps, dst_node)).to eq(TransactionChains::Vps::Replace::Os)
  end
  ...
  it 'rejects unsupported hypervisor combinations' do
    ...
    expect do
      ...
      /Replace from vpsadminos to openvz is not supported/
    )
  end
end
