...
module NotificationRoutingSpecHelpers
  ...
  def route_default_notifications_to_email_for!(user)
    receiver = default_email_receiver_for(user)
    EventRoute.default_route_for(user).update!(notification_receiver: receiver)
    user.set_notification_delivery_method!(:email, true)
    receiver
  end

  def mute_default_notifications_for!(user)
