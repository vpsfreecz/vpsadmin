...
RSpec.describe AddIpReleaseCampaigns do
  it 'creates the campaign tables and allows only one active claim per IP' do
    ...
    expect(table_exists?(:ip_release_attempts)).to be(true)
    membership = { ip_release_attempt_id: 1, ip_release_request_address_id: 1 }
    insert_row(:ip_release_attempt_addresses, membership)
    expect { insert_row(:ip_release_attempt_addresses, membership) }.to raise_error(ActiveRecord::RecordNotUnique)
    insert_row(:ip_release_attempt_addresses, membership.merge(ip_release_attempt_id: 2))
  ...
  it 'removes only the new tables on rollback' do
    migrate_up!
    migrate_down!
    expect(table_exists?(:ip_release_attempts)).to be(false)
    expect(table_exists?(:ip_release_attempt_addresses)).to be(false)
