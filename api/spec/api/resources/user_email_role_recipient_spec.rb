...
RSpec.describe 'VpsAdmin::API::Resources::User::EmailRoleRecipient' do
  before do
    header 'Accept', 'application/json'
  end

  def index_path(user_id)
    vpath("/users/#{user_id}/email_role_recipients")
  end

  def show_path(user_id, role)
    vpath("/users/#{user_id}/email_role_recipients/#{role}")
  end
  ...
  def recipients
    json.dig('response', 'email_role_recipients') || []
  end

  def recipient
    json.dig('response', 'email_role_recipient') || json['response']
  end
  ...
  describe 'API description' do
    it 'includes email role recipient endpoints' do
      scopes = EndpointInventory.scopes_for_version(self, api_version)
      expect(scopes).to include(
        'user.email_role_recipient#index',
        'user.email_role_recipient#show',
        'user.email_role_recipient#update'
      )
    end
  end
  ...
  describe 'Index' do
    ...
    it 'allows user to list their own role recipients' do
      UserEmailRoleRecipient.where(user: SpecSeed.user).delete_all
    ...
    it 'allows admin listing another user recipients' do
      UserEmailRoleRecipient.where(user: SpecSeed.other_user).delete_all
    ...
    it 'ignores persisted recipients for unregistered roles' do
      ...
      UserEmailRoleRecipient.where(user: SpecSeed.user, role:).delete_all
      legacy_recipient = UserEmailRoleRecipient.new(
        user: SpecSeed.user,
        role:,
  ...
  describe 'Show' do
    ...
    it 'returns 404 when no record exists for that role' do
      UserEmailRoleRecipient.where(user: SpecSeed.user, role: 'admin').delete_all
    ...
    it 'shows an existing configured recipient' do
      UserEmailRoleRecipient.where(user: SpecSeed.user, role: 'account').delete_all
      UserEmailRoleRecipient.create!(user: SpecSeed.user, role: 'account', to: 'acct@test.invalid')
    ...
    it 'denies user showing another user recipient' do
      UserEmailRoleRecipient.where(user: SpecSeed.other_user, role: 'account').delete_all
      UserEmailRoleRecipient.create!(user: SpecSeed.other_user, role: 'account', to: 'other@test.invalid')
  ...
  describe 'Update' do
    it 'rejects unauthenticated access' do
      json_put update_path(SpecSeed.user.id, 'account'),
               email_role_recipient: { to: 'x@test.invalid' }
    ...
    it 'allows user to create/update their own recipient' do
      ...
      as(SpecSeed.user) do
        json_put update_path(SpecSeed.user.id, 'account'),
                 email_role_recipient: { to: "a@test.invalid, b@test.invalid \n" }
      end
      ...
      recp = UserEmailRoleRecipient.find_by!(user: SpecSeed.user, role: 'account')
    ...
    it 'allows user to clear recipient by sending empty string' do
      ...
      expect(UserEmailRoleRecipient.where(user: SpecSeed.user, role: 'account')).to be_empty
    end
    ...
    it 'rejects invalid email address' do
      ...
      as(SpecSeed.user) do
        json_put update_path(SpecSeed.user.id, 'account'),
                 email_role_recipient: { to: 'notanemail' }
      end
    ...
    it 'rejects unregistered roles without persisting them' do
      ...
      as(SpecSeed.user) do
        json_put update_path(SpecSeed.user.id, role),
                 email_role_recipient: { to: 'attacker@example.test' }
      end
      ...
      expect(messages).to match(/not a registered email role/i)
      expect(UserEmailRoleRecipient.where(user: SpecSeed.user, role:)).to be_empty
    end
    ...
    it 'denies updating another user recipient' do
      ...
      expect(json['status']).to be(false)
      expect(response_message).to match(/access denied/i)
      expect(UserEmailRoleRecipient.where(user: SpecSeed.other_user, role: 'account')).to be_empty
    end
    ...
    it 'allows admin to update another user recipient' do
      ...
      as(SpecSeed.admin) do
        json_put update_path(SpecSeed.other_user.id, 'account'),
                 email_role_recipient: { to: 'admin@test.invalid' }
      end
      ...
      recp = UserEmailRoleRecipient.find_by!(user: SpecSeed.other_user, role: 'account')
