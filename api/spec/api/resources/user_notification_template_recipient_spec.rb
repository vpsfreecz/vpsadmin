...
RSpec.describe 'VpsAdmin::API::Resources::User::NotificationTemplateRecipient' do
  before do
    header 'Accept', 'application/json'
  end

  def index_path(user_id)
    vpath("/users/#{user_id}/notification_template_recipients")
  end

  def show_path(user_id, tpl_name)
    vpath("/users/#{user_id}/notification_template_recipients/#{tpl_name}")
  end
  ...
  def list
    json.dig('response', 'notification_template_recipients') || []
  end

  def obj
    json.dig('response', 'notification_template_recipient') || json['response']
  end
  ...
  let!(:templates) do
    ...
    UserNotificationTemplateRecipient.create!(
      user: user,
      notification_template: tpl_public_saved,
      to: 'saved1@test.invalid',
      enabled: true
    )
    UserNotificationTemplateRecipient.create!(
      user: user,
      notification_template: tpl_nonpub_visible,
      to: 'saved2@test.invalid',
      enabled: true
    )
    UserNotificationTemplateRecipient.create!(
      user: user,
      notification_template: tpl_invisible,
  ...
  describe 'API description' do
    it 'includes notification template recipient endpoints' do
      scopes = EndpointInventory.scopes_for_version(self, api_version)
      expect(scopes).to include(
        'user.notification_template_recipient#index',
        'user.notification_template_recipient#show',
        'user.notification_template_recipient#update'
      )
    end
  end
  ...
  describe 'Update' do
    it 'rejects unauthenticated access' do
      json_put update_path(user.id, tpl_public_saved.name),
               notification_template_recipient: { to: 'x@test.invalid' }
    ...
    it 'updates recipients and strips whitespace' do
      payload = { notification_template_recipient: { to: ' new@test.invalid , second@test.invalid ' } }
      ...
      recp = UserNotificationTemplateRecipient.find_by!(user: user, notification_template: tpl_public_saved)
      expect(recp.to).to eq('new@test.invalid,second@test.invalid')
    end

    it 'rejects invalid email address' do
      UserNotificationTemplateRecipient.where(user: user, notification_template: tpl_public_saved).delete_all

      as(user) do
        json_put update_path(user.id, tpl_public_saved.name),
                 notification_template_recipient: { to: 'invalid-email' }
      end
    ...
    it 'removes saved record when resetting to defaults' do
      expect(UserNotificationTemplateRecipient.where(user: user, notification_template: tpl_public_saved)).not_to be_empty

      payload = { notification_template_recipient: { to: '', enabled: true } }

      as(user) { json_put update_path(user.id, tpl_public_saved.name), payload }

      expect_status(200)
      expect(json['status']).to be(true)
      expect(UserNotificationTemplateRecipient.where(user: user, notification_template: tpl_public_saved)).to be_empty
    end

    it 'keeps a record when disabling template' do
      UserNotificationTemplateRecipient.where(user: user, notification_template: tpl_public_saved).delete_all

      payload = { notification_template_recipient: { to: '', enabled: false } }
    ...
    it 'denies updating another user recipient' do
      payload = { notification_template_recipient: { to: 'x@test.invalid' } }
    ...
    it 'rejects invisible template updates for normal users' do
      recp = UserNotificationTemplateRecipient.find_by!(user: user, notification_template: tpl_invisible)
      original = recp.to
      payload = { notification_template_recipient: { to: 'hidden-update@test.invalid' } }
    ...
    it 'allows admin to update another user recipient' do
      payload = { notification_template_recipient: { to: 'admin@test.invalid' } }
      ...
      recp = UserNotificationTemplateRecipient.find_by!(user: other_user, notification_template: tpl_public_saved)
      expect(recp.to).to eq('admin@test.invalid')
    end

    it 'allows admin to update invisible templates' do
      payload = { notification_template_recipient: { to: 'admin-hidden@test.invalid' } }
