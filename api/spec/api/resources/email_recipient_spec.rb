...
RSpec.describe 'VpsAdmin::API::Resources::EmailRecipient' do
  let!(:recipient_a) do
  ...
  def index_path
    vpath('/email_recipients')
  end

  def show_path(id)
    vpath("/email_recipients/#{id}")
  end
  ...
  def recipients
    json.dig('response', 'email_recipients')
  end

  def recipient
    json.dig('response', 'email_recipient') || json['response']
  end
  ...
  describe 'API description' do
    it 'includes email_recipient endpoints' do
      scopes = EndpointInventory.scopes_for_version(self, api_version)

      expect(scopes).to include(
        'email_recipient#index',
        'email_recipient#show',
        'email_recipient#create',
  ...
  describe 'Index' do
    ...
    it 'allows admin to list email recipients' do
    ...
    it 'supports pagination limit' do
      as(SpecSeed.admin) { json_get index_path, email_recipient: { limit: 1 } }
    ...
    it 'supports pagination from_id' do
      boundary = EmailRecipient.order(:id).first.id

      as(SpecSeed.admin) { json_get index_path, email_recipient: { from_id: boundary } }
    ...
    it 'returns meta count' do
      ...
      expect_status(200)
      expect(json['status']).to be(true)
      expect(json.dig('response', '_meta', 'total_count')).to eq(EmailRecipient.count)
    end
  end
  ...
  describe 'Show' do
    ...
    it 'allows admin to show a email recipient' do
    ...
    it 'returns 404 for unknown email recipients' do
  ...
  describe 'Create' do
    ...
    it 'rejects unauthenticated access' do
      json_post index_path, email_recipient: payload
    ...
    it 'forbids normal users' do
      as(SpecSeed.user) { json_post index_path, email_recipient: payload }

      expect_status(403)
      expect(json['status']).to be(false)
    end

    it 'allows admin to create a email recipient' do
      expect do
        as(SpecSeed.admin) { json_post index_path, email_recipient: payload }
      end.to change(EmailRecipient, :count).by(1)
      ...
      record = EmailRecipient.find(recipient['id'])
    ...
    it 'returns validation errors for missing label' do
      expect do
        as(SpecSeed.admin) { json_post index_path, email_recipient: payload.except(:label) }
      end.not_to change(EmailRecipient, :count)
  ...
  describe 'Update' do
    it 'rejects unauthenticated access' do
      json_put show_path(recipient_a.id), email_recipient: { label: 'Changed', to: nil }
    ...
    it 'forbids normal users' do
      as(SpecSeed.user) { json_put show_path(recipient_a.id), email_recipient: { label: 'Changed', to: nil } }

      expect_status(403)
      expect(json['status']).to be(false)
    end

    it 'allows admin to update a email recipient' do
      as(SpecSeed.admin) { json_put show_path(recipient_a.id), email_recipient: { label: 'Changed', to: nil } }
    ...
    it 'returns validation errors for blank label' do
      ...
      as(SpecSeed.admin) { json_put show_path(recipient_a.id), email_recipient: { label: '' } }
    ...
    it 'returns 404 for unknown email recipients' do
      ...
      as(SpecSeed.admin) { json_put show_path(missing), email_recipient: { label: 'Changed' } }
  ...
  describe 'Delete' do
    ...
    it 'allows admin to delete a email recipient' do
      expect do
      ...
      expect_status(200)
      expect(json['status']).to be(true)
      expect(EmailRecipient.find_by(id: to_delete.id)).to be_nil
    end
