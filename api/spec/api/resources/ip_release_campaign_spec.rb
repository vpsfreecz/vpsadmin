...
RSpec.describe VpsAdmin::API::Resources::IpReleaseCampaign do
  ...
  it 'shows notice history only to its owner and administrators' do
    ...
    as(SpecSeed.user) do
      ...
      notices = JSON.parse(last_response.body).fetch('response').fetch('notices')
      expect(notices.map { |notice| notice.except('_meta').keys.sort }.uniq)
        .to eq([%w[id event subject created_at].sort])
