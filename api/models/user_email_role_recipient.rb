class UserEmailRoleRecipient < ApplicationRecord
  belongs_to :user
  ...
  def self.all_roles_for(user)
    ...
    ::NotificationTemplate.roles.each_key do |role|
  ...
  def self.registered_roles
    ::NotificationTemplate.roles.keys.map(&:to_s)
  end
  ...
  def self.role_config(role)
    ::NotificationTemplate.roles.each do |name, opts|
  ...
  def self.check_role!(user, role)
    ...
    recp = new(user:, role:)
    recp.errors.add(:role, 'is not a registered email role')
  ...
  def check_role
    ...
    errors.add(:role, 'is not a registered email role')
  end
end
