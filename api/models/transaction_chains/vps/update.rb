  ...
  class Vps::Update < ::TransactionChain
    ...
    def check_ds_mounts(vps, dataset, user_id)
      dataset.dataset_in_pools.each do |dip|
        dip.mounts.each do |mnt|
          ...
          raise "dataset #{dataset.full_name} is mounted in VPS #{mnt.vps_id}"
        end
      end
    end
    ...
    def standalone_ips(vps, r)
      ...
      q.each do |ip|
        ...
        changes[ip] = { user_id: vps.user_id } if vps.node.location.environment.user_ip_ownership
      end
