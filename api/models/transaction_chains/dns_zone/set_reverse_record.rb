  ...
  class DnsZone::SetReverseRecord < ::TransactionChain
    ...
    def link_chain(host_ip_address, ptr_content)
      ...
      if record
        ...
        raise "Unable to find reverse record name for #{host_ip_address} in zone #{dns_zone.name}" unless /^(.+)\.#{Regexp.escape(dns_zone.name)}$/ =~ host_ip_address.reverse_record_domain
      ...
      dns_zone.dns_server_zones.each do |dns_server_zone|
        if created
          append_t(Transactions::DnsZone::CreateRecord, args: [dns_server_zone, record])
        else
          append_t(Transactions::DnsZone::UpdateRecord, args: [dns_server_zone, record])
        end
