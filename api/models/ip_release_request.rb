class IpReleaseRequest < ApplicationRecord
  ...
  delegate :deadline, :allow_keep, :closed_at, to: :ip_release_campaign
  ...
  def eligible_addresses
    ip_release_request_addresses.order(:id).select { |item| item.protection == 'eligible' }
  end

  def notice_available?(event)
    initially_notified = ip_release_request_notices.where(event: 'requested').exists?
    return false if (event == 'requested' && initially_notified) || (event == 'reminder' && !initially_notified)

    owner = original_user
    user_available?(owner) && owner.mailer_enabled
  end
