...
class StorageEffectRegistry
    ...
    groups.each do |group, (handles, admit)|
      ...
      handles.each do |handle|
        ...
        entries[handle] = Entry.new(
          ...
          handle == 5225 ? :api_only_unsupported : :paired,
  ...
  CHAIN_EFFECTS = {
    'TransactionChains::DatasetInPool::DetachBackupHeads' => :catalog_topology
  }.freeze

  def self.chain_admission_required?(klass)
    effect = CHAIN_EFFECTS[klass.name]
    if klass.storage_effect && effect != klass.storage_effect
      raise Unclassified, "unclassified storage chain #{klass.name}"
    end
