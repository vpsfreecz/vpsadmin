...
class member_load {
	...
	function create_new($item) {
		...
		if (!$this->exists) {
			...
			$this->m["m_info"] = addslashes("Created: ".date('Y-m-d', time()).' '.$item["m_info"]);
...
/*				mail(
					$db->check($this->m["m_mail"]),
					_("Account has been created."),
					sprintf(
						_("Dear new user,\n\nYour account has been created with following login information into vpsAdmin on %s.\n\nUsername: %s\nPassword: %s \n\nBest regards."),
						"alfa, https://alfa.vpsfree.cz/vpsadmin/",
						$db->check($this->m["m_nick"]),
						$item["m_pass"]
						   ),
