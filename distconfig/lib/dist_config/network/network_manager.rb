...
module DistConfig
  ...
  class Network::NetworkManager < Network::Base
    def usable?
      begin
    ...
    def do_create_connection(netif)
      tpl_base = File.join('network/network_manager')
    ...
    def generate_nm_conf(netifs)
      ...
      ErbTemplate.render_to_if_changed(
        File.join('network/network_manager/nm_conf'),
        { netifs: },
        file
      )
    end
    ...
    def generate_nm_udev_rules(netifs)
      ...
      ErbTemplate.render_to_if_changed(
        File.join('network/network_manager/udev_rules'),
        { netifs: },
        file
      )
    end
