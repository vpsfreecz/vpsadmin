...
require 'nodectld/commands/vps/apply_user_data'
require 'nodectld/vps_user_data'
require 'nodectld/vps_user_data/script'

RSpec.describe NodeCtld::Commands::Vps::ApplyUserData do
  let(:driver) { build_vps_driver }
  let(:handler) { class_double(NodeCtld::VpsUserData::Script, apply: nil) }
  let(:cmd) do
    described_class.new(
      driver,
      'vps_id' => 101,
  ...
  it 'dispatches application to the format-specific handler' do
    expect(cmd.exec).to eq(ret: :ok)
    expect(handler).to have_received(:apply).with(
      101,
      'script',
      "#!/bin/sh\necho spec\n",
