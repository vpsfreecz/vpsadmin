...
require 'nodectld/commands/dns_server_zone/remove_servers'
require 'nodectld/dns_config'
require 'nodectld/dns_server_zone'

RSpec.describe NodeCtld::Commands::DnsServerZone::RemoveServers do
  ...
  def zone_nameservers
    @zone_nameservers ||= ['ns1.example.test', 'ns2.example.test']
  end

  def zone_primaries
    @zone_primaries ||= [{ 'ip_addr' => '192.0.2.1' }, { 'ip_addr' => '192.0.2.10' }]
  end

  def zone_secondaries
    @zone_secondaries ||= [{ 'ip_addr' => '192.0.2.2' }, { 'ip_addr' => '192.0.2.20' }]
  end
  ...
  it 'removes the requested servers on exec and adds them back on rollback' do
